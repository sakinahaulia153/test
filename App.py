...
st.set_page_config(
    page_title="ResuMate Bot",
    page_icon='./Logo/RM_Bot.ico',
)


def run():
    st.title("ResuMate Bot")
    st.sidebar.markdown("# Choose User")
    activities = ["Normal User", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    # link = '[©Developed by Spidy20](http://github.com/spidy20)'
    # st.sidebar.markdown(link, unsafe_allow_html=True)
    img = Image.open('./Logo/RM_Bot.png')
    img = img.resize((700, 120))
    st.image(img)
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'resumate' and ad_password == 'abc123':
                st.success("Welcome Everyone")
                # Display Data
